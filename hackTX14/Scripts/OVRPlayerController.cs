...
public class OVRPlayerController : MonoBehaviour
{
	...
	// @Sammy -- UpdateMovement
	static float sDeltaRotationOldY = 0.0f;
	static float sDeltaRotationOldX = 0.0f;
	/// <summary>
...
// @Sammmy -- Start			
		...
		//@Sammy: commenting out to prevent rotation by touchpad
#if UNITY_EDITOR
		if(SkipMouseRotation == false) {
			deltaRotationY = Input.GetAxis("Mouse X") * rotateInfluence * 3.25f;

			//AJD adding in mouse y camera rotation for th desktop
			deltaRotationX = Input.GetAxis("Mouse Y") * rotateInfluence * -3.25f;
		}
#endif

		float filteredDeltaRotationY = (sDeltaRotationOldY * 0.0f) + (deltaRotationY * 1.0f);
		YRotation += filteredDeltaRotationY;
		sDeltaRotationOldY = filteredDeltaRotationY;

		//AJD lots of crap to get the mouse up and down working on the desktop
		float filteredDeltaRotationX = (sDeltaRotationOldX * 0.0f) + (deltaRotationX * 1.0f);
		XRotation += filteredDeltaRotationX;
		sDeltaRotationOldX = filteredDeltaRotationX;
		//end of AJD change
// @Sammy - End
	...
	public void SetCameras()
	{
		...
		{
			//AJD why aren't these camera rotations actually being reflected?
