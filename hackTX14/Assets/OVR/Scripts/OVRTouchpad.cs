	...
	static void HandleInput(TouchState state, ref Vector2 move)
	{
		...
		else if (touchState == TouchState.Move)
		{
			...
			if(Mathf.Abs(move.x) > Mathf.Abs (move.y))
			{
				...
				{
					OVRDebugStreamer.message = 
					System.String.Format("SWIPE: LEFT\n");		
				}
				else
				{
					OVRDebugStreamer.message = 
					System.String.Format("SWIPE: RIGHT\n");		
				}
			}
			// Right
			else
			{
				...
				{
					OVRDebugStreamer.message = 
					System.String.Format("SWIPE: DOWN\n");		
				}
				else
				{
					OVRDebugStreamer.message = 
					System.String.Format("SWIPE: UP\n");		
				}
			}
		}
	}
...
public sealed class OVRTouchpadHelper : MonoBehaviour 
{
	...
	void LocalTouchEventCallback(object sender, EventArgs args)
	{
		...
		switch(touchEvent)
		{
			...
			OVRDebugStreamer.message = System.String.Format("SINGLE CLICK\n");
			break;
			
			case(OVRTouchpad.TouchEvent.DoubleTap):
			OVRDebugStreamer.message = System.String.Format("DOUBLE CLICK\n");
			break;
			
			case(OVRTouchpad.TouchEvent.Left):
			OVRDebugStreamer.message = System.String.Format("LEFT SWIPE\n");
			break;
			
			case(OVRTouchpad.TouchEvent.Right):
			OVRDebugStreamer.message = System.String.Format("RIGHT SWIPE\n");
			break;
			
			case(OVRTouchpad.TouchEvent.Up):
			OVRDebugStreamer.message = System.String.Format("UP SWIPE\n");
			break;
			
			case(OVRTouchpad.TouchEvent.Down):
			OVRDebugStreamer.message = System.String.Format("DOWN SWIPE\n");
			break;
		}
	}
}
